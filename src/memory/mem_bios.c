...
int loadbios() {
	...
	switch (romset) {
	case ROM_PC1512:f = romfopen("pc1512/40043.v1", "rb");
		...
		return 1;
	case ROM_PC1640:f = romfopen("pc1640/40044.v3", "rb");
		...
		fclose(f);
		return 1;
	case ROM_PC200:f = romfopen("pc200/pc20v2.1", "rb");
		...
		return 1;
	case ROM_PPC512:f = romfopen("ppc512/40107.v2", "rb");
		...
		return 1;
	case ROM_TANDY:f = romfopen("tandy/tandy1t1.020", "rb");
		if (!f)
			break;
		romfread(rom, 65536, 1, f);
		fclose(f);
		return 1;
	case ROM_TANDY1000HX:f = romfopen("tandy1000hx/v020000.u12", "rb");
		if (!f)
			break;
		...
		biosmask = 0x1ffff;
		return 1;
	case ROM_TANDY1000SL2:f = romfopen("tandy1000sl2/8079047.hu1", "rb");
...
/*                case ROM_IBMPCJR:
                ...
                fclose(f);
                break;*/
	case ROM_IBMXT:f = romfopen("ibmxt/xt.rom", "rb");
		if (!f) {
	...
	case ROM_IBMPCJR:f = romfopen("ibmpcjr/bios.rom", "rb");
		if (!f)
			break;
	...
	case ROM_GENXT:f = romfopen("genxt/pcxt.rom", "rb");
		if (!f)
			break;
		romfread(rom + 0xE000, 8192, 1, f);
		fclose(f);
	...
	case ROM_CBM_PC10:f = romfopen("cbm_pc10/cbm-pc10c-bios-v4.41-318085-08.bin", "rb");
		if (!f)
			break;
	...
	case ROM_SUPER16T:f = romfopen("huyndaixt/super16t_v1.12ta.bin", "rb");
		if (!f)
			break;
	...
	case ROM_SUPER16TE:f = romfopen("huyndaixte/super16te_v2.00id.bin", "rb");
		if (!f)
			break;
		romfread(rom + 0xC000, 16384, 1, f);
		fclose(f);
	...
	case ROM_DTKXT:f = romfopen("dtk/dtk_erso_2.42_2764.bin", "rb");
		if (!f)
			break;
		romfread(rom + 0xE000, 8192, 1, f);
		fclose(f);
		return 1;
	case ROM_OLIM24:f = romfopen("olivetti_m24/olivetti_m24_version_1.43_low.bin", "rb");
	...
	case ROM_PC2086:f = romfopen("pc2086/40179.ic129", "rb");
	...
	case ROM_PC3086:f = romfopen("pc3086/fc00.bin", "rb");
		if (!f)
			break;
	...
	case ROM_PC5086:f = romfopen("pc5086/sys_rom.bin", "rb");
		if (!f)
			break;
		romfread(rom, 65536, 1, f);
		fclose(f);
...
/*                f=romfopen("amic206.bin","rb");
                ...
                fclose(f);
                return 1;*/
	case ROM_IBMAT386:f = romfopen("ibmat/62x0820.u27", "rb");
		...
		fclose(f);
		return 1;
	case ROM_CMDPC30:f = romfopen("cmdpc30/commodore pc 30 iii even.bin", "rb");
		...
		biosmask = 0x7fff;
		return 1;
	case ROM_DELL200:f = romfopen("dells200/dell0.bin", "rb");
...
//                f=romfopen("at386/at386.bin","rb");
	...
	case ROM_ACER386:f = romfopen("acer386/acer386.bin", "rb");
		if (!f)
			break;
		romfread(rom, 65536, 1, f);
		fclose(f);
	...
	case ROM_KMXC02:f = romfopen("kmxc02/3ctm005.bin", "rb");
		if (!f)
			break;
		romfread(rom, 65536, 1, f);
		fclose(f);
	...
	case ROM_AMI286:f = romfopen("ami286/amic206.bin", "rb");
		if (!f)
			break;
		romfread(rom, 65536, 1, f);
		fclose(f);
//                memset(romext,0x63,0x8000);
	...
	case ROM_TG286M:f = romfopen("tg286m/ami.bin", "rb");
		if (!f)
			break;
		romfread(rom, 65536, 1, f);
		fclose(f);
//                memset(romext,0x63,0x8000);
	...
	case ROM_AWARD286:f = romfopen("award286/award.bin", "rb");
		if (!f)
			break;
		romfread(rom, 65536, 1, f);
		fclose(f);
	...
	case ROM_GDC212M:f = romfopen("gdc212m/gdc212m_72h.bin", "rb");
		if (!f)
			break;
		romfread(rom, 65536, 1, f);
		fclose(f);
	...
	case ROM_HYUNDAI_SUPER286TR:f = romfopen("super286tr/award.bin", "rb");
		if (!f)
			break;
		romfread(rom, 65536, 1, f);
		fclose(f);
	...
	case ROM_GW286CT:f = romfopen("gw286ct/2ctc001.bin", "rb");
		if (!f)
			break;
		romfread(rom, 65536, 1, f);
		fclose(f);
	...
	case ROM_SPC4200P:f = romfopen("spc4200p/u8.01", "rb");
		if (!f)
			break;
		romfread(rom, 65536, 1, f);
		fclose(f);
	...
	case ROM_SPC4216P:f = romfopen("spc4216p/phoenix.bin", "rb");
		if (!f) {
	...
	case ROM_SPC4620P:f = romfopen("spc4620p/svb6120a_font.rom", "rb");
		if (!f)
			break;
		fclose(f);
	...
	case ROM_SPC6000A:f = romfopen("spc6000a/3c80.u27", "rb");
	...
	case ROM_SPC6033P:f = romfopen("spc6033p/svb6120a_font.rom", "rb");
		if (!f)
			break;
		fclose(f);
...
//                memset(romext,0x63,0x8000);
	...
	case ROM_IBMPC:f = romfopen("ibmpc/pc102782.bin", "rb");
		if (!f)
			break;
//                f=fopen("pc081682.bin","rb");
	...
	case ROM_MEGAPC:f = romfopen("megapc/41651-bios lo.u18", "rb");
	...
	case ROM_AMI486:f = romfopen("ami486/ami486.bin", "rb");
		if (!f)
			break;
		romfread(rom, 65536, 1, f);
		fclose(f);
...
//                f=romfopen("win486/win486.bin","rb");
	...
	case ROM_PCI486:f = romfopen("hot-433/hot-433.ami", "rb");
		if (!f)
			break;
		romfread(rom, 0x20000, 1, f);
		fclose(f);
	...
	case ROM_SIS496:f = romfopen("sis496/sis496-1.awa", "rb");
		if (!f)
			break;
		romfread(rom, 0x20000, 1, f);
		fclose(f);
	...
	case ROM_P55VA:f = romfopen("p55va/va021297.bin", "rb");
		if (!f)
			break;
		romfread(rom, 0x20000, 1, f);
		fclose(f);
	...
	case ROM_P55TVP4:f = romfopen("p55tvp4/tv5i0204.awd", "rb");
		if (!f)
			break;
		romfread(rom, 0x20000, 1, f);
		fclose(f);
...
//                f=romfopen("430vx/430vx","rb");
	...
	case ROM_P55T2P4:f = romfopen("p55t2p4/0207_j2.bin", "rb");
		if (!f)
			break;
		romfread(rom, 0x20000, 1, f);
		fclose(f);
	...
	case ROM_REVENGE:f = romfopen("revenge/1009af2_.bio", "rb");
		if (!f)
			break;
		...
		//is486=1;
		return 1;
	case ROM_ENDEAVOR:f = romfopen("endeavor/1006cb0_.bio", "rb");
		if (!f)
			break;
		fseek(f, 0x80, SEEK_SET);
		romfread(rom + 0x10000, 0x10000, 1, f);
...
//#endif
	...
	case ROM_IBMPS1_2121:f = romfopen("ibmps1_2121/fc0000.bin", "rb");
		if (!f)
			break;
	...
	case ROM_DESKPRO_386:f = romfopen("deskpro386/109592-005.u11.bin", "rb");
	...
	case ROM_AMIXT:f = romfopen("amixt/ami_8088_bios_31jan89.bin", "rb");
		if (!f)
			break;
		romfread(rom + 0xE000, 8192, 1, f);
		fclose(f);
	...
	case ROM_LTXT:f = romfopen("ltxt/27c64.bin", "rb");
		if (!f)
			break;
		romfread(rom + 0xE000, 8192, 1, f);
		fclose(f);
	...
	case ROM_LXT3:f = romfopen("lxt3/27c64d.bin", "rb");
		if (!f)
			break;
		romfread(rom + 0xE000, 8192, 1, f);
		fclose(f);
	...
	case ROM_PXXT:f = romfopen("pxxt/000p001.bin", "rb");
		if (!f)
			break;
		romfread(rom + 0xE000, 8192, 1, f);
		fclose(f);
	...
	case ROM_JUKOPC:f = romfopen("jukopc/000o001.bin", "rb");
		if (!f)
			break;
		romfread(rom + 0xE000, 8192, 1, f);
		fclose(f);
	...
	case ROM_IBMPS2_M30_286:f = romfopen("ibmps2_m30_286/33f5381a.bin", "rb");
		if (!f)
			break;
		romfread(rom, 0x20000, 1, f);
		fclose(f);
	...
	case ROM_IBMPS2_M50:f = romfopen("i8550021/90x7423.zm14", "rb");
	...
	case ROM_IBMPS2_M55SX:f = romfopen("i8555081/33f8146.zm41", "rb");
	...
	case ROM_IBMPS2_M80:f = romfopen("i8580111/15f6637.bin", "rb");
	...
	case ROM_ATARIPC3:f = romfopen("ataripc3/AWARD_ATARI_PC_BIOS_3.08.BIN", "rb");
		if (!f)
			break;
		romfread(rom + 0x8000, 32768, 1, f);
		fclose(f);
	...
	case ROM_IBMXT286:f = romfopen("ibmxt286/BIOS_5162_21APR86_U34_78X7460_27256.BIN", "rb");
	...
	case ROM_EPSON_PCAX:f = romfopen("epson_pcax/EVAX", "rb");
	...
	case ROM_EPSON_PCAX2E:f = romfopen("epson_pcax2e/EVAXE", "rb");
	...
	case ROM_EPSON_PCAX3:f = romfopen("epson_pcax3/EVAX3", "rb");
	...
	case ROM_T3100E:loadfont("t3100e/t3100e_font.bin", FONT_T3100E);
	...
	case ROM_T1000:loadfont("t1000/t1000font.rom", FONT_CGA);
	...
	case ROM_T1200:loadfont("t1200/t1000font.rom", FONT_CGA);
	...
	case ROM_PB_L300SX:f = romfopen("pb_l300sx/pb_l300sx.bin", "rb");
		if (!f)
			break;
		romfread(rom, 65536, 1, f);
		fclose(f);
	...
	case ROM_NCR_PC4I:f = romfopen("ncr_pc4i/NCR_PC4i_BIOSROM_1985.BIN", "rb");
		if (!f)
			break;
		romfread(rom, 0x4000, 1, f);
		fclose(f);
	...
	case ROM_TO16_PC:f = romfopen("to16_pc/TO16_103.bin", "rb");
		if (!f)
			break;
		romfread(rom + 0x8000, 32768, 1, f);
		fclose(f);
	...
	case ROM_COMPAQ_PII:f = romfopen("compaq_pii/109740-001.rom", "rb");
	...
	case ROM_ZAPPA:f = romfopen("zappa/1006bs0_.bio", "rb");
		if (!f)
			break;
		fseek(f, 0x80, SEEK_SET);
		romfread(rom + 0x10000, 0x10000, 1, f);
...
//                        break;
	...
	case ROM_COMPAQ_PIP:f = romfopen("compaq_pip/Compaq Portable Plus 100666-001 Rev C.bin", "rb");
		if (!f)
			break;
		romfread(rom + 0xE000, 8192, 1, f);
		fclose(f);
	...
	case ROM_XI8088:f = romfopen("xi8088/bios-xi8088.bin", "rb"); /* use the bios without xt-ide because it's configurable in pcem */
		if (!f)
			break;
	...
	case ROM_IBMPS2_M70_TYPE3:f = romfopen("ibmps2_m70_type3/70-a_even.bin", "rb");
	...
	case ROM_IBMPS2_M70_TYPE4:f = romfopen("ibmps2_m70_type4/70-b_even.bin", "rb");
	...
	case ROM_TULIP_TC7:f = romfopen("tulip_tc7/tc7be.bin", "rb");
	...
	case ROM_PB410A:f = romfopen("pb410a/PB410A.080337.4ABF.U25.bin", "rb");
		if (!f)
			break;
		romfread(rom, 0x20000, 1, f);
		fclose(f);
	...
	case ROM_BULL_MICRAL_45: f = romfopen("bull_micral_45/even.fil", "rb");
	...
	case ROM_FIC_VA503P:f = romfopen("fic_va503p/je4333.bin", "rb");
		if (!f)
			break;
		romfread(rom, 0x20000, 1, f);
		fclose(f);
	...
	case ROM_CBM_SL386SX25:f = romfopen("cbm_sl386sx25/f000.rom", "rb");
		if (!f)
			break;
		romfread(rom, 0x10000, 1, f);
		fclose(f);
	...
	case ROM_IBMPS1_2133_451:f = romfopen("ibmps1_2133/PS1_2133_52G2974_ROM.bin", "rb");
		if (!f)
			break;
		romfread(rom, 0x20000, 1, f);
		fclose(f);
	...
	case ROM_ECS_386_32: f = romfopen("ecs386_32/386_32_even.bin", "rb");
	...
	case ROM_LEDGE_MODELM:f = romfopen("leadingedge_modelm/Leading Edge - Model M - BIOS ROM - Version 4.71.bin", "rb");
		if (!f)
			break;
		romfread(rom, 0x4000, 1, f);
		fclose(f);
	...
	case ROM_DESKPRO:f = romfopen("deskpro/Compaq - BIOS - Revision J - 106265-002.bin", "rb");
		if (!f)
			break;
	...
	case ROM_VS440FX:f = romfopen("vs440fx/1018CS1_.BIO", "rb");
		if (!f)
			break;
		fseek(f, 0x80, SEEK_SET);
		romfread(rom + 0x10000, 0x10000, 1, f);
	...
	case ROM_GA686BX:f = romfopen("ga686bx/6BX.F2a", "rb");
		if (!f)
			break;
