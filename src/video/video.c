...
VIDEO_CARD v_olivetti_go481 = {"Olivetti GO481 (Paradise PVGA1A)", "olivetti_go481", &paradise_pvga1a_oli_go481_device, GFX_OLIVETTI_GO481, VIDEO_FLAG_TYPE_SPECIAL,
...
device_t *video_card_getdevice(int card, int romset) {
	switch (romset) {
	case ROM_IBMPCJR:return &pcjr_video_device;

	case ROM_TANDY:
	case ROM_TANDY1000HX:return &tandy_device;

	case ROM_TANDY1000SL2:return &tandysl_device;

	case ROM_PC1512:return &pc1512_device;
	...
	case ROM_OLIM24:return &m24_device;
	...
	case ROM_MEGAPC:return &paradise_wd90c11_megapc_device;
	...
	case ROM_ACER386:return &oti067_acer386_device;

	case ROM_AMA932J:return &oti067_ama932j_device;

	case ROM_IBMPS1_2011:
	case ROM_IBMPS2_M30_286:return &ps1vga_device;
	...
	case ROM_IBMPS1_2121:return &ps1_m2121_svga_device;

	case ROM_IBMPS1_2133_451:return &gd5426_ps1_device;

	case ROM_T3100E:return &t3100e_device;

	case ROM_T1000:
	case ROM_T1200:return &t1000_device;

	case ROM_ELX_PC425X:return &tgui9400cxi_elx_device;

	case ROM_PB410A:return &ht216_32_pb410a_device;
...
void video_updatetiming() {
	if (video_speed == -1) {
		...
		case ROM_TANDY:
		case ROM_TANDY1000HX:
		case ROM_TANDY1000SL2:timing = &timing_dram;
			break;

		case ROM_PC1512:timing = &timing_pc1512;
			break;
		...
		case ROM_OLIM24:timing = &timing_m24;
			break;
		...
		case ROM_IBMPS1_2011:
		case ROM_IBMPS2_M30_286:timing = &timing_vga;
			break;
		...
		case ROM_IBMPS1_2121:timing = &timing_ps1_svga;
			break;

		case ROM_IBMPS1_2133_451:timing = &timing_pb570;
			break;

		case ROM_T3100E:timing = &timing_t3100e;
			break;

		case ROM_T1000:timing = &timing_t1000;
			break;

		case ROM_ELX_PC425X:timing = &timing_pc425x;
			break;
		...
		case ROM_PB570:timing = &timing_pb570;
			break;
...
void video_init() {
	...
	switch (romset) {
	case ROM_IBMPCJR:device_add(&pcjr_video_device);
		return;

	case ROM_TANDY:
	case ROM_TANDY1000HX:device_add(&tandy_device);
		return;

	case ROM_TANDY1000SL2:device_add(&tandysl_device);
		return;

	case ROM_PC1512:device_add(&pc1512_device);
		return;
	...
	case ROM_OLIM24:device_add(&m24_device);
		return;
	...
	case ROM_ACER386:device_add(&oti067_acer386_device);
		if (gfxcard != GFX_BUILTIN) {
	...
	case ROM_AMA932J:device_add(&oti067_ama932j_device);
		return;

	case ROM_IBMPS1_2011:
	case ROM_IBMPS2_M30_286:device_add(&ps1vga_device);
		return;
	...
	case ROM_IBMPS2_M70_TYPE3:
	case ROM_IBMPS2_M70_TYPE4:
	case ROM_IBMPS2_M80:device_add(&ps1vga_device);
		if (gfxcard == GFX_BUILTIN)
			return;
		break;

	case ROM_IBMPS1_2121:device_add(&ps1_m2121_svga_device);
		return;

	case ROM_IBMPS1_2133_451:device_add(&gd5426_ps1_device);
		return;

	case ROM_T3100E:device_add(&t3100e_device);
		return;

	case ROM_T1000:
	case ROM_T1200:device_add(&t1000_device);
		return;

	case ROM_ELX_PC425X:device_add(&tgui9400cxi_elx_device);
		return;

	case ROM_PB410A:device_add(&ht216_32_pb410a_device);
		if (gfxcard != GFX_BUILTIN) {
			svga_set_override(svga_get_pri(), 1);
			break;
		}
	...
	case ROM_PB570:device_add(&gd5430_pb570_device);
		if (gfxcard != GFX_BUILTIN) {
			svga_set_override(svga_get_pri(), 1);
			break;
		}
   ...
   /* TODO */
	...
	case ROM_CBM_SL386SX25:device_add(&avga2_cbm_sl386sx_device);
		return;
	}
...
void cgapal_rebuild(int display_type, int contrast) {
	...
	default:cgapal[0x0] = makecol(0x00, 0x00, 0x00);
